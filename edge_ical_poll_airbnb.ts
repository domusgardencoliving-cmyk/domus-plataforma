// =========================================================
// EDGE FUNCTION: ical-poll-airbnb
//
// Lê iCal feed do Airbnb (1 por listing) e UPSERT no DG.
// Sem precisar de Channel Manager.
//
// Onde pegar o iCal do Airbnb:
//   1. Acessar Airbnb → Anúncios → [escolher anúncio] → Calendário
//   2. Sincronização → "Exportar calendário"
//   3. Copiar URL → salvar como AIRBNB_ICAL_<UH> nos secrets Supabase
//
// Limitação: Airbnb mascara o nome do hóspede no iCal — geralmente vem como
// "Reserved" ou "(Not available)" + ID externo. O nome real só vem por email
// quando o hóspede confirma. Usaremos o gmail-poll do Átrio pra completar.
//
// Cron: 5min, junto com Booking
// =========================================================
...
const isReservaReal = (summary: string): boolean => {
  ...
  // Airbnb marca períodos bloqueados como "Not available"
  return !s.includes("not available") && !s.includes("blocked") && !s.includes("airbnb (not available)");
};

const extractCodigoReservaAirbnb = (description?: string): string | null => {
  // Airbnb iCal description tipicamente tem:
  // Reservation URL: https://www.airbnb.com/hosting/reservations/details/HMABCD1234
  if (!description) return null;
  const m = description.match(/details\/([A-Z0-9]+)/i);
  return m ? m[1] : null;
};
...
Deno.serve(async (_req) => {
  ...
  for (const [k, v] of Object.entries(env)) {
    if (k.startsWith("AIRBNB_ICAL_") && v) {
      const partes = k.replace("AIRBNB_ICAL_", "").split("_");
  ...
  if (icalUrls.length === 0) {
    return new Response(JSON.stringify({
      ok: false,
      erro: "Nenhuma URL iCal Airbnb configurada (AIRBNB_ICAL_*)",
      como_configurar: "Airbnb → Anúncios → Calendário → Sincronização → Exportar calendário. Salvar URL nos Supabase secrets.",
  ...
  for (const { url, cama, unidade } of icalUrls) {
    try {
      ...
      for (const ev of eventos) {
        ...
        const codigoAirbnb = extractCodigoReservaAirbnb(ev.description);
        const externoId = `AIRBNB_${codigoAirbnb || ev.uid}`;
        const checkin = isoFromYYYYMMDD(ev.dtstart);
        const checkout = isoFromYYYYMMDD(ev.dtend);
        // Airbnb mascara o nome — fica "Hóspede Airbnb" até o email parser preencher
        const nome = ev.summary && !ev.summary.toLowerCase().includes("reserved")
          ? ev.summary
          : `Hóspede Airbnb (${codigoAirbnb || ev.uid.slice(0, 8)})`;
        ...
        if (!existente) {
          await sb.from("reservas").insert({
            ...
            origem_dados: "ical_airbnb",
            ultima_sync_hospedin: new Date().toISOString(),
          });
          stats.criadas++;
          acoes.push({ acao: "criada", nome, cama, checkin, checkout, codigo: codigoAirbnb });
